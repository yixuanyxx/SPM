...
# Table name for saved reports
REPORTS_TABLE = "report"

class SupabaseReportRepo:
    def __init__(self):
    ...
    def save_report(self, report_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        res = self.client.table(REPORTS_TABLE).insert(report_data).execute()
        if not res.data:
    ...
    def get_report(self, report_id: int) -> Optional[Dict[str, Any]]:
        ...
        res = self.client.table(REPORTS_TABLE).select("*").eq("id", report_id).single().execute()
        return res.data

    def get_reports_by_user(self, user_id: int) -> List[Dict[str, Any]]:
        """Get all reports generated by a specific user"""
        res = self.client.table(REPORTS_TABLE).select("*").eq("generated_by", user_id).order("created_at", desc=True).execute()
        return res.data or []

    def get_reports_by_type(self, report_type: str, user_id: Optional[int] = None) -> List[Dict[str, Any]]:
        """Get reports by type, optionally filtered by user"""
        query = self.client.table(REPORTS_TABLE).select("*").eq("report_type", report_type)
        if user_id:
            query = query.eq("generated_by", user_id)
        res = query.order("created_at", desc=True).execute()
        return res.data or []

    def get_team_reports(self, team_id: int) -> List[Dict[str, Any]]:
        """Get all reports for a specific team"""
        res = self.client.table(REPORTS_TABLE).select("*").eq("team_id", team_id).order("created_at", desc=True).execute()
        return res.data or []

    def get_department_reports(self, dept_id: int) -> List[Dict[str, Any]]:
        """Get all reports for a specific department"""
        res = self.client.table(REPORTS_TABLE).select("*").eq("dept_id", dept_id).order("created_at", desc=True).execute()
        return res.data or []

    def delete_report(self, report_id: int) -> bool:
        """Delete a report by ID"""
        res = self.client.table(REPORTS_TABLE).delete().eq("id", report_id).execute()
        return len(res.data) > 0

    def update_report(self, report_id: int, updates: Dict[str, Any]) -> Dict[str, Any]:
        """Update a report"""
        res = self.client.table(REPORTS_TABLE).update(updates).eq("id", report_id).execute()
        if not res.data:
