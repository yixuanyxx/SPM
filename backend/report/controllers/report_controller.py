...
@report_bp.route("/reports/company/<int:admin_user_id>", methods=["GET"])
def generate_company_report(admin_user_id: int):
    """
    Generate company-wide report showing all departments, teams, and members.
    ...
    {
        "status": 200,
        "message": "Company report generated",
        "data": {
            "company_report": {
                "departments": [ ... department reports with teams and members ... ]
            }
        }
    }
    ...
    RESPONSES:
        200: Report successfully generated
        403: User is not authorized to generate company reports
        500: Internal Server Error
    """
    try:
        from datetime import datetime
        ...
        # Generate report data with date filtering
        result = report_service.generate_company_report(admin_user_id, start_date=start_date, end_date=end_date)
        ...
        if export_format == 'json':
            ...
            company_data = result["data"]["company_report"]
            pdf_data = export_service.export_company_report_pdf(company_data)
            
            return send_file(
                io.BytesIO(pdf_data),
                mimetype='application/pdf',
                as_attachment=True,
                download_name=f'company_report_{datetime.now().strftime("%Y%m%d")}.pdf'
            )
            
        elif export_format == 'excel':
            company_data = result["data"]["company_report"]
            excel_data = export_service.export_company_report_excel(company_data)
            
            return send_file(
                io.BytesIO(excel_data),
                mimetype='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
                as_attachment=True,
                download_name=f'company_report_{datetime.now().strftime("%Y%m%d")}.xlsx'
            )
