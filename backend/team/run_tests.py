...
- Team Model tests (data validation)
- Team Controller tests (API endpoints)
"""
...
# def run_model_tests():
#     """Run team model tests (no environment variables needed)."""
#     print("\nRunning Team Model Tests...")
#     try:
...
def run_controller_tests():
    """Run team controller integration tests."""
    print("\nRunning Team Controller Tests...")
    try:
        # Add current directory (microservice root) to Python path and run tests
        env = os.environ.copy()
        env['PYTHONPATH'] = os.getcwd()
        ...
        result = subprocess.run([sys.executable, "-m", "unittest", 
                               "test_team_controller_integration", "-v"],
                              cwd="tests", capture_output=True, text=True, env=env)
...
def main():
    """Main function."""
    print("Simple Test Runner - Controller Only")
    print("=" * 50)
