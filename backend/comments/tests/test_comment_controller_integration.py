...
class TestCommentControllerIntegration(unittest.TestCase):
    ...
    def cleanup_test_data(self):
        ...
        try:
            ...
            test_task_ids = [1561, 1562, 999]  # Common test task IDs
            deleted_count = 0
    ...
    def test_create_comment_success(self):
        ...
        self.assertEqual(data["data"]["task_id"], 1561)
    ...
    def test_get_task_comments_success(self):
        ...
        # Make request
        response = self.client.get('/comments/task/1561')
    ...
    def test_get_comment_with_all_fields(self):
        ...
        self.assertEqual(comment_data["task_id"], 1561)
    ...
    def test_comments_different_tasks(self):
        ...
        self.client.post('/comments/create', json={
            "task_id": 1561,
            "user_id": 297,
            "content": "Comment for task 1561"
        })
        
        self.client.post('/comments/create', json={
            "task_id": 1562,
            "user_id": 297,
            "content": "Comment for task 1562"
        })
        
        # Get comments for task 1
        response1 = self.client.get('/comments/task/1561')
        self.assertEqual(response1.status_code, 200)
        data1 = json.loads(response1.data)
        self.assertEqual(len(data1["data"]), 1)
        self.assertEqual(data1["data"][0]["content"], "Comment for task 1561")
        
        # Get comments for task 2
        response2 = self.client.get('/comments/task/1562')
        self.assertEqual(response2.status_code, 200)
        data2 = json.loads(response2.data)
        self.assertEqual(len(data2["data"]), 1)
        self.assertEqual(data2["data"][0]["content"], "Comment for task 1562")
