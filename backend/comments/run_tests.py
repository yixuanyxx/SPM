...
- Comment Model tests (data validation)
- Comment Controller tests (API endpoints)
"""
...
def run_model_tests():
    """Run comment model tests (no environment variables needed)."""
    print("\nRunning Comment Model Tests...")
    try:
...
def run_controller_tests():
    """Run comment controller integration tests."""
    print("\nRunning Comment Controller Tests...")
    try:
        # Add current directory (microservice root) to Python path and run tests
        env = os.environ.copy()
        env['PYTHONPATH'] = os.getcwd()
        ...
        result = subprocess.run([sys.executable, "-m", "unittest", 
                               "test_comment_controller_integration", "-v"],
                              cwd="tests", capture_output=True, text=True, env=env)
