...
- User Model tests (data validation)
- User Controller tests (API endpoints)
"""
...
def run_model_tests():
    """Run user model tests (no environment variables needed)."""
...
def run_controller_tests():
    """Run user controller integration tests."""
    ...
    try:
        ...
        result = subprocess.run([sys.executable, "-m", "unittest", 
                               "test_user_controller_integration", "-v"],
                              cwd="tests", capture_output=True, text=True, env=env)
