    ...
    <div class="app-container">
      ...
      <div class="header-section">
        ...
        <div class="header-actions">
          <div class="header-left-actions" v-if="deptId && !selectedTeamId">
            <button class="back-btn" @click="goBackToDepartments">
      ...
      <div v-if="!deptId" class="departments-grid-container">
        <div v-if="isLoadingDepartments" class="loading-state">
        ...
        <div v-else class="departments-container">
            <div
                ...
                @click="selectDepartment(department.id, department.name)"
                >
                <div class="department-header">
                    ...
                    <div class="department-meta">
                        <div class="meta-item">
                            ...
                            <span>{{ getDepartmentTeamCount(department.id) }} teams</span>
                        </div>
                        <div class="meta-item">
                            <i class="bi bi-people"></i>
                            <span>{{ getDepartmentMemberCount(department.id) }} members</span>
                        </div>
                        <div class="meta-item">
                            <i class="bi bi-list-task"></i>
                            <span>{{ getDepartmentTaskCount(department.id) }} tasks</span>
                        </div>
                    </div>
                </div>
            </div>
            <div class="department-action">
                <span class="action-text">Click to view teams</span>
      ...
      <div v-else-if="!selectedTeamId" class="teams-grid-container">
        <!-- Loading state -->
...
const pageTitle = computed(() => {
  if (selectedTeamId.value) {
    return `${selectedTeamName.value} Workload`
  } else if (deptId.value) {
    return `${selectedDepartmentName.value} Teams`
  } else {
...
const pageSubtitle = computed(() => {
  if (selectedTeamId.value) {
    return 'Monitor and manage team member task distribution'
  } else if (deptId.value) {
    return 'Select a team to view member workloads and tasks'
  } else {
    return 'Select a department to view their teams'
  }
})
  ...
  if (userId.value) {
    try {
      ...
      if (response.ok) {
        ...
        await fetchAllMembersGlobal()
        await fetchAllTasksGlobal()
...
const fetchAllMembersGlobal = async () => {
  try {
    const response = await fetch(`http://localhost:5003/users/all`) // all members endpoint
    if (response.ok) {
      const data = await response.json()
      allMembers.value = data.data || []
      console.log('Fetched all members globally:', allMembers.value.length)
    } else {
      console.error('Failed to fetch all members globally:', response.status)
    }
  } catch (err) {
    console.error('Error fetching all members globally:', err)
  }
}

// Fetch all tasks for all teams (for pre-selection counts)
const fetchAllTasksGlobal = async () => {
  try {
    const response = await fetch(`http://localhost:5002/tasks`) // all tasks endpoint
    if (response.ok) {
      const data = await response.json()
      
      // Ensure users are fetched
      if (!allMembers.value.length) {
        console.error('All users not loaded yet')
        return
      }
    ...
    console.error('Error fetching all tasks globally:', err)
  }
}
...
const fetchAllDepartmentMembers = async () => {
  ...
  try {
    ...
    if (response.ok) {
      const data = await response.json()
      console.log('Department members raw data:', data)
      allDepartmentMembers.value = data.data || []
      console.log('All department members:', allDepartmentMembers.value)
...
const fetchAllDepartments = async() =>{
    ...
    try {
        const response = await fetch(`http://localhost:5005/departments`)
        const data = await response.json()
        departments.value = data.data || []  
        console.log('Fetched all departments for stats')
    } catch (error) {
...
const fetchAllTeams = async () => {
  try {
    const response = await fetch(`http://localhost:5004/teams`) // all teams
    if (!response.ok) throw new Error(`Failed to fetch all teams`)
        const data = await response.json()
        allTeams.value = data.data || []
        console.log('Fetched all teams:', allTeams.value.length)
  } catch (err) {
    console.error(err)
    allTeams.value = []
  }
...
const departmentTeamsMap = computed(() => {
  ...
  departments.value.forEach(dep => {
    map[dep.id] = allTeams.value
      .filter(team => String(team.dept_id) === String(dep.id))
      .map(team => String(team.id)) // use string IDs for consistency
  })
  console.log('departmentTeamsMap:', map)
  return map
})
...
.department-card:hover .department-action {
  color: #3b82f6;
}
